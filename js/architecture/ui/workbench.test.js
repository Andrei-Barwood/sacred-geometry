...
test("data sources are plain rows Alpine can iterate; only one dialog at a time", () => {
  const wb = createWorkbench();
  wb.openSources();
  assert.ok(wb.sourceRows.length >= 1, "expected registered sources");
  assert.equal(typeof wb.sourceRows[0].id, "string");
  assert.equal(typeof wb.sourceRows[0].organization, "string");
  assert.equal(Object.isFrozen(wb.sourceRows[0]), false);
  wb.ui.reportOpen = true;
  wb.openSources();
  assert.equal(wb.ui.sourcesOpen, true);
  assert.equal(wb.ui.reportOpen, false);
});
