...
var DEFAULT_ALLOWED_METHODS = "GET, POST, OPTIONS";
...
// functions/_utils/subscriptions.js
async function listAllSubscriptions(kv) {
  const results = [];
  let cursor;
  do {
    const page = await kv.list({ cursor, limit: 1e3 });
...
function parseSubscription(raw) {
  ...
  try {
    ...
    if (!data || typeof data.endpoint !== "string" || !data.keys || typeof data.keys.p256dh !== "string" || typeof data.keys.auth !== "string") {
      return null;
    }
    return {
...
// scripts/src/notify.pages.js
var GONE_STATUSES = /* @__PURE__ */ new Set([404, 410]);
var SEND_CONCURRENCY = 10;
...
async function onRequestPost(context) {
  ...
  if (!env.VAPID_PUBLIC_KEY || !env.VAPID_PRIVATE_KEY || !env.VAPID_SUBJECT) {
    return jsonResponse(
      ...
      {
        ok: false,
        error: "Faltan variables VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY o VAPID_SUBJECT."
      },
      500
    );
  }
  ...
  try {
    ...
    return jsonResponse(
      request,
      { ok: false, error: "JSON inv\xE1lido en el cuerpo de la petici\xF3n." },
      400
    );
  }
  ...
  const icon = typeof body?.icon === "string" && body.icon.trim() ? body.icon.trim() : "/favicon.jpeg";
  if (!title) {
    return jsonResponse(
      request,
...
async function onRequest(context) {
  ...
  return jsonResponse(
    context.request,
    { ok: false, error: "M\xE9todo no permitido. Usa POST." },
    405
  );
}
  ...
  await mapPool(rows, SEND_CONCURRENCY, async (row) => {
    ...
    try {
      ...
      if (stats.errors.length < 10) {
        stats.errors.push(
          `${res.status} ${res.statusText} \xB7 ${subscription.endpoint.slice(0, 80)}`
        );
      }
    } catch (err) {
